...
export default function EditListing() {
    ...
    mutationFn: async (data: any) => {
      ...
      if (images.length > 0) {
        for (let i = 0; i < images.length; i++) {
          const file = images[i];
          const fileName = `${id}_${Date.now()}_${i}.${file.name.split('.').pop()}`;
  ...
  const deleteImage = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["listing-images", id] });
  ...
  if (error || !listing) {
    return (
      <div className="container mx-auto py-8">
        <p className="text-center text-destructive">Anúncio não encontrado ou erro ao carregar.</p>
      </div>
    );
  }
  ...
  return (
    <>
      <SEOHead 
        title={`Editar Anúncio: ${listing.title} - tudofaz.com`}
        description="Edite seu anúncio e mantenha suas informações atualizadas"
      />
      ...
      <div className="container mx-auto py-8">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold">Editar Anúncio: {listing.title}</h1>
        </div>
